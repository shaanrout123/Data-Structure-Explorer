...
public class DoublyLinkedListVisualizer extends JPanel {
    ...
    protected void paintComponent(Graphics g) {
        ...
        for (int i = 0; i < list.size(); i++) {
            // Draw the info box
            g.drawRect(startX + i * width, startY - height / 2, width / 3, height);
            g.drawString("Info", startX + i * width + 10, startY);

            // Draw the data box
            g.drawRect(startX + i * width + width / 3, startY - height / 2, width / 3, height);
            g.drawString(list.get(i).toString(), startX + i * width + width / 3 + 15, startY);

            // Draw the next box
            g.drawRect(startX + i * width + 2 * width / 3, startY - height / 2, width / 3, height);
            g.drawString("Next", startX + i * width + 2 * width / 3 + 10, startY);

            // Draw the index label
            g.drawString("Pos " + (i + 1), startX + i * width + 20, startY - height);

            // Draw the next arrow for forward linkage
            if (i < list.size() - 1) {
            ...
            // Draw the previous arrow for backward linkage
            if (i > 0) {
                g.drawLine(startX + i * width, startY, startX + (i - 1) * width + width, startY);
                g.drawLine(startX + i * width - 10, startY - 10, startX + i * width, startY);
                g.drawLine(startX + i * width - 10, startY + 10, startX + i * width, startY);
            }
        }
    }
    ...
    public static void main(String[] args) {
        ...
        /* SwingUtilities.invokeLater(() -> {
            JFrame frame = new JFrame("Doubly Linked List Visualizer");
            DoublyLinkedListVisualizer visualizer = new DoublyLinkedListVisualizer();
